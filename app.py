...
def dashboard():
    ...
    total_buyback = int(sum(float(d.get('BuybackPrice') or 0) for d in devices))
    
    # Calculate simulated actuals since we don't store exact historical profit in DB
    total_revenue = int(sum(float(d.get('BuybackPrice') or 0) * 1.35 for d in devices))
    total_profit = int(sum(float(d.get('BuybackPrice') or 0) * 0.25 for d in devices))
...
def customer_profile(customer_id):
    ...
    total_value = int(sum(float(d.get('BuybackPrice') or 0) for d in devices))
